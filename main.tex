...
Esimerkiksi yritys jolla ei ole resursseja tai haluja palkata erikseen mobiilikehittäjiä on niin sanotusta cross-platform kehittämisestä tullut suosittua. \cite{biorn2017progressive} Sovelluskehityksen aika pienenee kun sovellusta ei tarvitse kehittää erikseen jokaiselle laitealustalle jolloin tuote saadaan nopeammin markkinoille.
...
Google on julkaissut Progressiivisille Web-sovelluksille tarkastuslistan, jota noudattamalla sovelluksesta saadaan progressiivinen \cite{Google}. Käydään seuraavaksi listasta läpi minimivaatimukset joiden tulee täyttyä, jotta sovellusta voidaan sanoa progressiiviseksi.
...
PWAstats \cite{PWAstats} on julkaissut tilastoja miten yritysten nykyisten sivujen muuttaminen PWA malliin on tuottanut lisätuloja tai käyttäjiä. Tämän työn tarkoituksena on selvittää mitä hyötyjä PWA malli tuo mukanaan ja onko PWA sovellus nopeampi kuin normaali verkkosovellus.
...
Jotta Service Workerin hyödyistä ja PWA sovelluksen luomisesta saataisiin parempi käsitys luotiin olemassa olevasta sovelluksesta PWA sovellus. Työssä päätettiin jättää vertailu muiden teknologioiden kanssa pois, sillä sovelluksen muuttaminen muille teknologioille sopivaksi olisi ollut liian haasteellista tai aikaa vievää. Tarkoituksena on esitellä PWA:n konseptia ja siihen liittyviä teknisiä vaatimuksia. 
