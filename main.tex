...
Joulukuussa 2018 projektin kehitystyökaluja haluttiin päivittää. Projektin kasaava Grunt työkalu on kehitetty vuonna 2012 ja se haluttiin korvata yksinkertaisemmalla konfiguraatiolla ja samalla siirtyä nykyaikaiseen modulaariseen kehitykseen. Nykyinen malli ei salli uusien JavaScript pakettien lataamista ja niiden tuomista projektiin helposti. Uudeksi build työkaluksi valittiin Webpack, joka on tarkoitettu modulaarista ja nykyaikaista kehittämistä varten. Paketinhallintatyökaluksi päätettiin vaihtaa jälleen NPM:n, eli Node Package Manageriin, joka on ottanut Yarnin kehityksessä kiinni. 
...
Sovellusta auditoitiin suoraan loppupalvelimelta. Vaihtoehtona olisi ollut auditoida sivua myös kehittäjän koneelta, mutta tällöin ei saada oikeita tuloksia loppukäyttäjän näkökulmasta. Esimerkiksi serverin nopeutta ei voida testata, sekä serverin hyödyntämä GZIP toiminto, joka paketoi skriptit pienemmiksi ei ole käytössä. Ensiksi auditoitiin sovelluksen versiota 1.7.0 joka oli kasattu vanhalla Grunt työkalulla, sekä vanhalla Service Worker skriptillä. 

\begin{figure}[h]
\begin{center}
\epsfig{figure=audit1_grunt.png,  width=0.5\textwidth}
...
Webpackin hyödyt nähtiin heti. Pelkästään siirtämällä nämä muutamat skriptit ladattavaksi moduuleina, saatiin sovelluksen lataamista nopeammaksi. Sovelluksen "Time to Interactive" laski yli sekunnin. Myös "First Meaningful Paint" tippui yli 0,5 sekuntia. Web-sovelluksista puhuttaessa on kyse huomattavasti ajasta. Sovelluksen kokonaiskäynnistymisaika on kuitenkin edelleen hidas verrattuna hyvin optimoituihin nykyaikaisiin modulaarisiin sivustoihin. Sekunnon nopeusmuutos antoi kuitenkin täydet 100 pistettä PWA kategoriasta. Sovellus siis täyttää kaikki PWA mallin kriteerit. Sovelluksen nopeutta mitattiin vielä uudestaan, jotta saataisiin tulokset Service Workerin muodostamasta välimuistin käytöstä.
...
Service Workerin tärkein toiminta liittyy suorituskykyyn, tiedostojen tallentamiseen välimuistiin. Web-sovellus joka lataa välimuistiin kaiken tiedon sivulta, pitäisi olla huomattavasti nopeampi kuin sovellus, joka ei hyödynnä välimuistia [Walton]. Waltonin artikkelissa Googlen developer dokumentaatiossa Case Studies osastolla on mitattu Google Analyticsiä hyödyntäen Googlen omaa I/O web sovelluksen nopeutta. Hyödyntämällä Google Analytics työkalua yhdessä Service Workerin kanssa saadaan vastaus, miten Service Workerin käyttö näkyy loppukäyttäjälle. Pelkästään Lighthouse työkalua hyödyntämällä ei saada tarpeeksi vastauksia. Telia IoT hankkeessa ei kuitenkaan keretty tehdä tarpeellisia analytiikkamittauksia. Käytetään Googlen mittaustuloksia kertomaan, miten sovelluksen käyttö eroaa loppukäyttäjälle. Kysymykset ovat seuraavat.
...
Kuvaajasta 5.5 nähdään, että Service Workerin hallitsema välimuisti (sininen pylväs) lataa sivut nopeammin kuin käyttäjillä, joilla on selaimen välimuistissa jo tietoa (punainen pylväs).
